...
for (let bash in batmanConfig) {
	if (batmanConfig.hasOwnProperty(bash)) {
		for (let name in batmanConfig[bash]) {
			if (batmanConfig[bash].hasOwnProperty(name)) {
				const currentCommand = batmanConfig[bash][name];
				const env = parseEnvs(currentCommand.env || []);
				commands[name] = {
					bash,
					name,
					...
					options: parseOptions(currentCommand.options || {}, env)
				}
			}
		}
	}
