...
const writeFile = (absPath, file) => {
    return fse.ensureFile(absPath)
        .then(() => fse.writeFile(absPath, typeof file === 'object' ? JSON.stringify(file) : file));
};
/**
 * End: FileSystem
 */
...
const getAbsPath = (...paths) => path.resolve(APP_PATH, ...paths);
const resolvePath = (...paths) => path.resolve(...paths);
const extname = (file) => path.extname(file).substr(1);
/**
 * End: Paths
 */
/**
...
const stringify = (obj, spaces) => JSON.stringify(obj, null, spaces || 4)
...
const printCommand = (command, name, isFull) => {
    if (isFull && typeof command === 'object') {
        console.log(`${colors.green('batman-name')}: %s ${colors.cyan('exec')}: %s ${colors.yellow('envs')}: %j`, name, command.fullCommand, command.envs);
    } else if (typeof command === 'object') {
        console.log(`${colors.green('batman-name')}: %s ${colors.cyan('desc')}: %s`, name, command.description);
    } else {
        console.log(`${colors.green('npm-name')}: %s ${colors.cyan('full')}: %s`, name, command);
    }
};
const printCommands = (isFull, ...commands) => {
    commands.forEach((currentTypeOfCommnads) => {
        for (let [command, name] of toArrayV2(currentTypeOfCommnads)) {
            printCommand(command, name, isFull);
        }
    });
};
/**
